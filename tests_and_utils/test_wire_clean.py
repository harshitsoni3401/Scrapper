...
async def test_wire_feeds():
    ...
    feeds = {
        ...
        "PR Newswire (Energy RSS)": "https://www.prnewswire.com/rss/energy-latest-news/energy-latest-news-list.rss",
    ...
    for name, url in feeds.items():
        try:
            ...
            if feed.entries:
                res.append(f"✅ RSS {name}: Found {len(feed.entries)} entries.")
            else:
                res.append(f"❌ RSS {name}: Returned 0 entries or failed.")
        except Exception as e:
            res.append(f"❌ RSS {name}: Exception {e}")
        ...
        for u in urls:
            try:
                ...
                async with session.get(u, headers=headers, timeout=10) as resp:
                    ...
                    if "robot" in title.lower() or "captcha" in title.lower() or resp.status != 200:
                         res.append(f"❌ HTML {u} [{resp.status}] CAPTCHA/BLOCK! Title: {title.strip()}")
                    else:
                         res.append(f"✅ HTML {u} [{resp.status}] SUCCESS! Title: {title.strip()}")
            except Exception as e:
                res.append(f"❌ HTML Exception for {u}: {e}")
